...
function updateButtonStates(quiz, index) {
    ...
    document.getElementById('forwardBtn').classList.toggle('hidden', index >= quiz.questions.length - 1);
    
    // Скрываем кнопку "К списку тестов", если мы не на последнем вопросе
    document.getElementById('toListBtn').classList.toggle('hidden', index < quiz.questions.length - 1);
}
